...
function run(trigger, scope, data, config, callback) {
    ...
    if (pConf && pConf.enable) {
        const message = {
            ...
            priority: pConf.priority ? parseInt(pConf.priority, 10) : 1,
        };

        let url = config.URL;
        if (!/^https+:\/\//.test(url))
            // URL doesn't start with http(s)
            url = `https://${url}`; // Add https:// if not present
        if (config.port && !Number.isNaN(config.port)) url = `${url}:${parseInt(config.port, 10)}`; // Add port to the end
        url += '/message'; // Append /message to URL
