angular.module('angular-highlight', []).directive('highlight', () => {
    const component = function (scope, element, attrs) {
        ...
        function rexReplacer(inputHtml) {
            ...
            scope.replacement.forEach((textreplace) => {
                ...
                if (thisMode == 'replace') {
                    ...
                    const match = html.match(textreplace.match);
                    ...
                    if (thisMode) {
                        html = html.replace(
                            thisRex,
                            `<a href="/?q=${match}" data-toggle="popover" class="${attrs.highlightClass}" title="${
                                textreplace.replace
                            }" onmouseenter="$(this).tooltip('show')">${match}</a>`
                        );
                    } else {
                        html = html.replace(
                            thisRex,
                            `<a href="/?q=${match}" data-toggle="popover" title="${
                                textreplace.replace
                            }" onmouseenter="$(this).tooltip('show')">${match}</a>`
                        );
                    }
