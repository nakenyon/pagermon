angular.module('app', ['ngRoute', 'ngResource', 'ngSanitize', 'angular-uuid', 'ui.bootstrap', 'color.picker', 'ui.validate', 'textAngular', 'ngFileSaver', 'angular-sortable-view'])
    ...
    .controller('AliasDetailCtrl', ['$scope', '$routeParams', 'Api', '$uibModal', '$filter', '$location', '$timeout', function ($scope, $routeParams, Api, $uibModal, $filter, $location, $timeout) {
      ...
      $scope.alertMessage = {};
      // Resolves when GET /admin/settingsData lands. Assigned at the bottom of this
      // controller; aliasLoad() waits on it for the plugin list.
      var settingsPromise;
      $scope.colorOptions = {
          required: true,
      ...
      // The plugin list arrives on a different request to the alias itself, and the
      // two now run in parallel, so this runs once both have landed.
      function seedPluginConf() {
        if (!$scope.alias) return;
        if (!$scope.alias.pluginconf) $scope.alias.pluginconf = {};
      ...
      $scope.aliasLoad = function() {
        ...
        Api.AliasDetail.get({id: $routeParams.id }, function(results) {
          ...
          // still clears the spinner and renders the form without plugin config.
          settingsPromise['finally'](function () {
            seedPluginConf();
            $scope.loading = false;
          });
        });
      ...
      // get data on load. These two requests are independent, so they go out
      // together rather than the alias fetch waiting on the settings callback.
      settingsPromise = Api.Settings.get(null, function(results) {
        if (results) {
          // settingsData returns {settings, plugins, themes}, so the flag lives at
          // results.settings.database - reading results.database meant this was
          // always undefined and the Refresh Aliases button never appeared here.
          // AliasController has always read the correct path.
          if (results.settings && results.settings.database &&
        ...
        // Previously a failure here meant aliasLoad() was never reached and the
        // page sat empty forever. Render the form without plugin config instead.
        $scope.plugins = [];
        console.error('settingsData load failed', err);
      }).$promise;
      $scope.aliasLoad();
      ...
      //
      // Measured on a live instance: rendering this grid with ng-repeat cost
      // ~12.5s of blocked main thread on every alias page load, whether or not
      // the picker was ever opened. Angular's share of that was 12ms. The rest
      // was the FontAwesome SVG kit, whose MutationObserver had to process 1001
      // elements inserted one at a time - most of them twice, because ng-repeat
      // inserts an unresolved `fa-{{faIcon}}` class and rewrites it on the next
      // digest, and an unresolved class matches no icon.
      //
      // So the grid is plain DOM: built on first open, one write per chunk, with
      // final class names already in the markup. Chunking keeps any single write
      // short enough that the modal stays usable while the rest fills in behind
      // the scroll fold.
      var FA_CHUNK = 100;
      var faBuilt = false;
      var faChunkTimer = null;
      var faFilterTimer = null;

      function faBlockHtml(name) {
        return '<div class="faHelperBlock" title="' + name + '" data-icon="' + name +
               '" onclick="setFaIcon(this);" onmouseover="showFaName(this);"' +
               ' onmouseleave="document.getElementById(\'faPreview\').innerText = \'\';"' +
               ' data-dismiss="modal"><i class="faHelperIcon fas fa-' + name + '"></i></div>';
      }

      // Build detached, then attach the whole fragment in one call, so the SVG
      // observer sees a single batch rather than one record per icon.
      function faAppend(grid, names) {
        var holder = document.createElement('div');
        holder.innerHTML = names.map(faBlockHtml).join('');
        var frag = document.createDocumentFragment();
        while (holder.firstChild) frag.appendChild(holder.firstChild);
        grid.appendChild(frag);
      }

      function faRender(names) {
        var grid = document.getElementById('faHelperGrid');
        if (!grid) return;
        if (faChunkTimer) { $timeout.cancel(faChunkTimer); faChunkTimer = null; }
        grid.innerHTML = '';
        faAppend(grid, names.slice(0, FA_CHUNK));
        var next = FA_CHUNK;
        function appendNext() {
          faAppend(grid, names.slice(next, next + FA_CHUNK));
          next += FA_CHUNK;
          faChunkTimer = next < names.length ? $timeout(appendNext, 0, false) : null;
        }
        if (names.length > FA_CHUNK) faChunkTimer = $timeout(appendNext, 0, false);
      }

      // Same behaviour as the `| filter: faSearch` this replaces: case-insensitive
      // substring match.
      function faApplyFilter(term) {
        var t = (term || '').trim().toLowerCase();
        faRender(t ? $scope.faIcons.filter(function (n) { return n.indexOf(t) !== -1; })
                   : $scope.faIcons);
      }
      ...
      function faBuild() {
        if (faBuilt) return;
        faBuilt = true;
        faRender($scope.faIcons);
        var input = document.getElementById('faSearch');
        if (!input) return;
        input.addEventListener('input', function () {
          var value = this.value;
          if (faFilterTimer) $timeout.cancel(faFilterTimer);
          // Debounced - each re-render is fresh work for the SVG observer, so
          // typing a word should cost one render, not one per keystroke.
          faFilterTimer = $timeout(function () { faApplyFilter(value); }, 200, false);
        });
      }

      // show.bs.modal rather than shown.bs.modal, so the first chunk renders
      // during Bootstrap's fade instead of after it.
      var faShowHandler = function () { faBuild(); };
      angular.element(document).on('show.bs.modal', '.faHelperModal', faShowHandler);

      $scope.$on('$destroy', function () {
        // Delegated off document, so without this every visit to an alias page
        // leaves a live handler behind holding a dead scope.
        angular.element(document).off('show.bs.modal', '.faHelperModal', faShowHandler);
        if (faChunkTimer) $timeout.cancel(faChunkTimer);
        if (faFilterTimer) $timeout.cancel(faFilterTimer);
      });
    }])
